...
class GoogleDriveRawDataSource {
  ...
  async refreshDriveNow({ silent = true } = {}) {
    ...
    try {
      ...
      // The "Trend" workbook feeds the visit-score trend chart and nothing else.
      // It is removed from every other candidate list here so it can never be
      // mistaken for a visit plan, a response export or an attendance file.
      const trendMeta = window.TrendSource?.pickTrendFile?.(metas) || null;
      const scheduleMetas = metas
        .filter(meta => meta !== responseMeta && meta !== trendMeta && scheduleFilePriority(meta) < 99)
        .sort((a, b) => scheduleFilePriority(a) - scheduleFilePriority(b) || Date.parse(b.modifiedTime || "") - Date.parse(a.modifiedTime || ""));
      const attendanceMetas = folderMetas.filter(meta => meta !== trendMeta && isAttendanceFilename(meta)).slice(0, 12);
      const signature = [
        PC_READER_VERSION,
        ...
        trendMeta ? this.drive.remoteSignature(trendMeta) : "no-trend",
      ].join("|");
      ...
      // Read the Trend workbook last and never let it break the refresh: it is
      // additive, so a missing or malformed Trend file must still leave every
      // compliance figure updating exactly as before.
      let trendPayload = null;
      if (trendMeta && window.TrendSource) {
        try {
          this.setStatus({ kind: "reading", message: `Reading visit-score trend from ${trendMeta.name}…` });
          const trendFile = await this.drive.downloadFile(trendMeta);
          trendPayload = window.TrendSource.toPayload(
            window.TrendSource.parseWorkbook(await window.TrendSource.toArrayBuffer(trendFile)),
            trendMeta.name,
          );
        } catch (error) {
          console.warn("Trend workbook could not be read:", error?.message || error);
        }
      }
      await this.applyResponseFile(responseFile, scheduleFile, signature, parsedResponse, parsedSchedule, parsedAttendance, folder, trendPayload);
      this.startWatching();
    } catch (error) {
  ...
  async applyResponseFile(responseFile, scheduleFile, signature, parsedResponse, parsedSchedule, parsedAttendance, folder, trendPayload = null) {
    let schedule;
    ...
    // Carried alongside the payload so every viewer gets the trend chart with no
    // Google Drive access at all. Only the trend card reads this key — no
    // compliance or audit figure looks at it. A failed Trend read keeps the last
    // good one rather than blanking the chart for everybody.
    const carriedTrend = trendPayload || this.currentData?.trend || null;
    if (carriedTrend) data.trend = carriedTrend;
